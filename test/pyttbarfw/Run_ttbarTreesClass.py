...
parser.add_option("-i", "--inDir", dest = "inDir", default = "./TTrees/",
                  help = "the input directory of root files" )
parser.add_option("-t", "--inTreeName", dest = "inTreeName", default = "TTreeSemiLeptSkim",
                  help = "the name of the input tree inside the input file" )
parser.add_option("-o", "--outFileName", dest = "outFileName", default = "ttbar_outfile.root",
...
###
chain = TChain(options.inTreeName,options.inTreeName )

chain.AddFile(options.inDir + "ttbarTuneCUETP8M2T4_highmass_METmu40el80ptRel30.root" )
...
if not options.load:
  ...
  gSystem.CompileMacro("ttbarTreesClass.C", "gOck")
  print"Done Compiling."

### Now we can load our class into our pyROOT session:
gSystem.Load("ttbarTreesClass_C")

### HATS comment:
# now we create an instance of our class
###
instance = ttbarTreesClass(chain)
