...
class B2GSelectSemiLepTTbar_Type2( ) :
    ...
    def select( self ) :
        ...
        for iptbin, ptbin in enumerate(self.ak8Jet_Ptbins) :
            if ptbin <  1000.:
                if (  ptbin < self.ak8PuppiSDJetP4_Subjet0.Perp() < self.ak8Jet_Ptbins[iptbin+1] ) :
                    self.ak8PuppiJetP4_Binned[iptbin].SetPtEtaPhiM( self.tree.JetPuppiPtRaw[0], self.tree.JetPuppiEtaRaw[0], self.tree.JetPuppiPhiRaw[0], self.tree.JetPuppiMassRaw[0] ) 
        ...
        if self.verbose: print"Stage 10: Leptonic top decay selection passed"
        ...
        if self.verbose: print"Stage 11: AK8 Pt>200 GeV, eta < 2.4 and hemisphere cut with lepton > 1."
        ...
        if not ( 10. < self.ak8PuppiSDJetP4.M() < 120. ) : return self.passed
        self.passed[3] = True
